...
class PyWeiboLoginAPI(object):
    ...
    def login(self, user_name, pass_word):
        ...
        self.session.headers.update({"Referer":login_referer})
        ...
        login_res_data=self.session.post(login_url,data=post_data).json()
        ...
        if login_res_data["retcode"]=="0":
            auth1_post_data={
                ...
                "userId":"",
                "passwd":""}
            auth1_res=self.session.post("https://api.weibo.com/oauth2/authorize",data=auth1_post_data)
            ...
            if auth2_res.status_code==302:
                redirecturl=auth2_res.headers["Location"].replace("http:","https:")
...
zhihu_csrf=weibo.session.get("http://www.zhihu.com")
weibo.csrf_string=zhihu_csrf.cookies["_xsrf"]
